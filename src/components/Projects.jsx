      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="flex justify-center mb-8 sm:mb-12 flex-wrap gap-2 animate-slide-in-left">
          {filterButtons.map((button) => (
            <button
              key={button.id}
              className={`px-3 sm:px-5 py-2 rounded-full m-1 transition-all hover:shadow-lg text-xs sm:text-sm font-medium transform hover:scale-105 ${
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filteredProjects.map((project, index) => (
            <div 
              key={project.id} 
              className="bg-zinc-800/50 p-4 sm:p-6 rounded-xl shadow-xl card-3d cursor-pointer border border-zinc-700 hover:border-emerald-400/50 transition-all duration-300 glass-card group animate-scale-in"
